...
def generate_data_set_with_varying_t_length():
    if not os.path.exists('data/feasible_sets'):
        os.makedirs('data/feasible_sets')

    instance_with_t_25 = DatasetGenerator.generate_multiple_feasible_instances(1, "Small", 10, 25, 2)
    instance_with_t_50 = DatasetGenerator.generate_multiple_feasible_instances(1, "Small", 10, 50, 2)
    instance_with_t_75 = DatasetGenerator.generate_multiple_feasible_instances(1, "Small", 10, 75, 2)
    instance_with_t_100 = DatasetGenerator.generate_multiple_feasible_instances(1, "Small", 10, 100, 2)
    instance_with_t_250 = DatasetGenerator.generate_multiple_feasible_instances(1, "Small", 10, 250, 2)
    instances = instance_with_t_25 + instance_with_t_50 + instance_with_t_75 + \
                instance_with_t_100 + instance_with_t_250

    with open("data/feasible_sets/instances_with_t_from_25_to_250_set.json", "w") as f:
        json.dump({
            "dataset_name": "instances_with_t_from_25_to_250_set",
            "instances": [instance.to_dict() for instance in instances]
        }, f, indent=4)
