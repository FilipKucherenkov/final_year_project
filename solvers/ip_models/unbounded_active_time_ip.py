...
def solve_unbounded_active_time_ip(instance: ParsedInstance, solver_type: str):
    """
    ...
    model.g = pyo.Var(model.timeslots, within=pyo.PositiveIntegers)
    ...
    def objective_rule(model):
        return sum(model.y[t] for t in model.timeslots) + sum(model.g[t] for t in model.timeslots)
    ...
    def parallel_job_assignment_rule(model, t):
        return sum(model.x[t, j] for j in model.jobs) <= model.y[t] * model.g[t]
